<script lang="ts">
   ...
   async function onActionResolved(actionId: string, outcome: string, actorName: string, checkType?: string, skillName?: string) {
      console.log(`ActionsPhase: onActionResolved`, { actionId, outcome, actorName, checkType, skillName });
      ...
      if (controller.isActionResolved(actionId)) {
         console.log(`ActionsPhase: Action ${actionId} already resolved`);
         return;
      }
      ...
      if (!action) {
         console.warn(`ActionsPhase: Action ${actionId} not found`);
         return;
      }
