...
Preview badges appear in the check card when it first mounts, showing players what could happen for each outcome. There are two types of badges:

1. **Static Preview Badges** - Defined in `outcomes`, shown before rolling
2. **Dynamic Outcome Badges** - Generated by `preview.calculate()`, shown after rolling

### Static Preview Badges

Add `outcomeBadges` array to any outcome in the pipeline's `outcomes` section:
...
- `textBadge(message, icon, variant)` - Simple text badge
- `valueBadge(template, icon, amount, variant)` - Badge with static number
- `diceBadge(template, icon, formula, variant)` - Badge with dice formula
- `badge({ template, icon, value, context, variant })` - Full control
...
For badges that depend on the roll outcome or need calculation, use `preview.calculate()`:

```typescript
export const myPipeline = createActionPipeline('my-action', {
  preview: {
    calculate: async (ctx) => {
      // Only show badge on critical failure
      if (ctx.outcome !== 'criticalFailure') {
        return { resources: [], outcomeBadges: [], warnings: [] };
      }
      
      // Generate dynamic badge based on actual state
      const handler = new DestroyWorksiteHandler();
      ...
      return {
        resources: [],
        outcomeBadges: [preparedCommand.outcomeBadge],  // Shows specific worksite
        warnings: []
      };
    }
  }
...
| Badge Type | Use When | Example |
|------------|----------|---------|
| **Static** (outcomes) | Effect is predictable and doesn't depend on game state | "1 random worksite destroyed", "Army gains +1 initiative" |
| **Dynamic** (preview.calculate) | Effect depends on current game state or roll outcome | "Quarry at Hex 419 destroyed", "Fort Tuskwater loses 10 HP" |

### Complete Example

The bandit-raids incident shows both types:
...
export const banditRaidsPipeline: CheckPipeline = {
  ...
  preview: {
    calculate: async (ctx) => {
      ...
      // DYNAMIC: Shows after rolling, with specific details
      const handler = new DestroyWorksiteHandler();
      const preparedCommand = await handler.prepare(
        { type: 'destroyWorksite', count: 1 },
        ctx
...
**User Experience:**
1. Check card opens → Shows "1 random worksite destroyed" (static badge)
2. User rolls critical failure → Badge updates to "Worksite Destroyed: Quarry" (dynamic badge)
3. User clicks "Apply Result" → Map displays destroyed worksite location

### Technical Details

**Flow:**
1. `buildPossibleOutcomes()` extracts `outcomeBadges` from pipeline's `outcomes`
2. `PossibleOutcomes.svelte` displays them in the check card
3. After roll, `preview.calculate()` returns additional dynamic badges
4. `OutcomeDisplay.svelte` shows combined badges (static + dynamic)

**Type Support:**
- `Outcome` interface includes `outcomeBadges?: any[]`
- `OutcomeEffects` interface includes `outcomeBadges` in all outcome types
- Works for actions, events, and incidents
