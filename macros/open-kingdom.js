// Macro to open PF2e Kingdom Lite UI
...
(async () => {
    ...
    // Try global object
    else if (game.pf2eKingdomLite?.openKingdomUI) {
        openKingdomUI = game.pf2eKingdomLite.openKingdomUI;
    }
    // Fallback for development
    else if (window.pf2eKingdomLite?.openKingdomUI) {
        openKingdomUI = window.pf2eKingdomLite.openKingdomUI;
    }
    // Legacy fallback - try to import the module directly
    else {
        try {
            // For production builds, try to import from the built module
            const modulePath = '/modules/pf2e-kingdom-lite/dist/index.js';
            ...
            console.error("PF2e Kingdom Lite | Failed to import module:", importError);
        }
    }
    ...
    if (openKingdomUI) {
        ...
        ui.notifications.error("PF2e Kingdom Lite module is active but UI function not found! Try refreshing the page.");
        console.error("PF2e Kingdom Lite | Available access points:", {
            module,
            moduleAPI: module?.api,
            moduleOpenKingdomUI: module?.openKingdomUI,
            gamePf2eKingdomLite: game.pf2eKingdomLite,
            windowPf2eKingdomLite: window.pf2eKingdomLite,
            windowOpenKingdomUI: window.openKingdomUI
        });
        console.log("PF2e Kingdom Lite | Hint: Make sure the dev server is running (npm run dev) and refresh Foundry after it starts.");
    }
})();
